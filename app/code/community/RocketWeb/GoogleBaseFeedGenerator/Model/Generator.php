...
class RocketWeb_GoogleBaseFeedGenerator_Model_Generator extends Varien_Object
{
    ...
    public function getProductMapModel($product, $args = array())
    {
        ...
        $file = ucfirst($key);
        if ($product->isConfigurable() && $this->getConfig()->isSimplePricingEnabled($product)) {
            $key .= '_scp';
            $file .= DS. 'Scp';
        }
        if ($is_assoc) {
    ...
    public function processProductCallback($args)
    {
        ...
        if ($this->getData('verbose')) {
            echo $this->formatMemory($memoryUsage) . " - SKU " . $args['row']['sku'] . ", ID " . $args['row']['entity_id'] . "\n";
        }

        if ($this->getBatchMode() && $this->isMemoryOverLimit()) {
            // We are terminating early
            $this->getBatch()->updateLock($this->_current_iter);
            $this->getBatch()->releaseLock();

            $this->releaseLock();

            $this->log('EARLY END / ALL MEMORY USED: ' . $this->formatMemory(memory_get_usage(true)));
            throw new Exception('Ran out of memory, waiting for next batch run');
        }
    ...
    {
        $memory_limit = Mage::helper('googlebasefeedgenerator')->getMemoryLimit();
        $units = array('b', 'Kb', 'Mb', 'Gb', 'Tb', 'Pb');
        $m = @round($memory / pow(1024, ($i = floor(log($memory, 1024)))), 2);
        $limit = @round($memory_limit / pow(1024, ($j = floor(log($memory_limit, 1024)))), 2);
