...
class ManaPro_FilterGroup_Model_Observer {
    ...
    public function getGroups($observer) {
        ...
        switch (Mage::getStoreConfig('mana_filters/advanced/group_by')) {
            case '':
                $groups = $groupHelper->getNoGroups($groups, $block->getFilters());
                break;
            case 'attribute_group':
                $groups = $groupHelper->getAttributeGroups($groups, $block->getFilters());
                break;
        }
